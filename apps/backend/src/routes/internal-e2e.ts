...
internalE2eRouter.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      // Support tickets (migration 061) + their messages, attachments and state
      // history (all FK cascades). Must run BEFORE the auth_users delete below:
      ...
      //
      // MARKER-SCOPED, NOT WHOLE-TENANT, unlike the notification and audit_log
      // sweeps next to it. Those wipe the pinned tenant because every row in
      // them is generated by the run; a ticket is a CONVERSATION SOMEBODY WROTE,
      // and the pinned tenant is also where product testing happens by hand. The
      // first version of this deleted the tenant's tickets wholesale and took
      // hand-made QA requests with it on 2026-08-25. So: fixture subjects
      // ('e2e-…', which every ticket spec uses) plus anything raised by an e2e
      // fixture user, wherever it lives.
      //
      // The audit rows the console wrote about them go first; they are keyed on
      // target_id, and the actor is often a REAL platform admin rather than a
      // fixture account, so the actor sweep below would not catch them.
      const tkq = await client.query(
        `SELECT a.r2_key
           FROM support_ticket_attachments a
           JOIN support_tickets t ON t.id = a.ticket_id
          WHERE (t.tenant_id = $2 AND t.subject LIKE 'e2e-%') OR t.user_id ${inE2eUsers}`,
        argsT
      );
          ...
          WHERE target_type = 'ticket'
            AND target_id IN (
                  SELECT id FROM support_tickets
                   WHERE (tenant_id = $2 AND subject LIKE 'e2e-%') OR user_id ${inE2eUsers}
                )`,
        argsT
      );
