...
export interface EmptyStateProps {
   ...
   */
  icon?: ReactNode;
  /** Primary call to action, e.g. "Nuovo cantiere". */
  action?: ReactNode;
  /**
   ...
   *
   * A reserved min-height plus centring inside it, rather than `margin: auto`
   * against an ancestor: the pages here wrap their content in ordinary blocks,
   * so an auto margin would have nothing to push against. When the block is
   * taller than the reserve it simply grows and nothing is cut off.
   */
  fill?: boolean;
  className?: string;
  'data-testid'?: string;
 ...
 *
 * No surface of its own: this block used to be a white dashed box dropped on
 * every page, which drew a panel around a sentence saying nothing is there. A
 * frame adds no information to "empty", and on a page whose only content IS the
 * empty state it reads as a small card marooned at the top of a large screen.
 * The box is gone, `fill` centres what is left, and the illustration — not a
 * grey glyph in a round chip — carries the weight the panel used to.
 */
