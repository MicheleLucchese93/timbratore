...
export const MAIN_EN = `
    ...
    <section class="chapter" id="web-admin-dashboard">
      ...
      <div class="feature">
        ...
        <p>Two columns show you who is absent right now and who will be over the next two weeks. For each entry: absence type, name, date range and total hours. For a half day (leave or time-off with a specific time window), <strong>Absent now</strong> shows the time band the person is away (e.g. 09:00–13:00) instead of “Until…”.</p>
      </div>
    ...
    <section class="chapter" id="web-admin-timbrature">
      ...
      <div class="feature">
        ...
        <ul class="tidy">
          ...
          <li><strong>Email</strong>, <strong>First name</strong>, <strong>Last name</strong> and <strong>Unique identifier</strong> of the employee.</li>
          ...
          <li><strong>Notes</strong> — any annotations. A <em>mock</em> indicator appears if the GPS position is suspicious, and <em>out of area</em> if the stamp was recorded outside the branch area (clock-out, breaks or lunch).</li>
          <li><strong>Actions</strong> — history, day dossier, edit and delete.</li>
        </ul>
        <p>The <strong>Show deleted</strong> checkbox (next to the filters) also lists deleted stamps: they appear struck through, with the <em>deleted</em> badge, and stay read-only.</p>
      </div>

      <div class="feature">
        <h3>Modified stamps: what you see and where</h3>
        <p>When an administrator moves a stamp's time, the original value — what the employee actually stamped — is <strong>never overwritten</strong>: it stays available forever. That is what a contestation, a payroll consultant's check or a labour inspection turns on.</p>
        <ul class="tidy">
          <li>The <strong>When</strong> column shows the <strong>original time</strong> in amber, in brackets: e.g. <em>20/07/2026, 09:03 (08:47)</em>.</li>
          <li>A <strong>modified</strong> badge sits next to the source; hover it to see who and when.</li>
          <li>In the <strong>Monthly grid</strong>, touched days show a small <strong>✎</strong> in the corner of the cell.</li>
        </ul>
        <p>Only a change of <strong>time</strong> or <strong>event type</strong> counts as a modification: fixing a note or the branch does not flag the stamp.</p>
      </div>

      <div class="feature">
        <h3>Stamp history and Day dossier</h3>
        <p>Two tools, both reachable from the row's action icons (and from the day editor in the Monthly grid):</p>
        <ul class="tidy">
          <li><strong>Stamp history</strong> (clock icon) — the full trail of that single stamp: the value the employee stamped, the current value and, row by row, every intervention with <em>date</em>, <em>type</em> (employee stamp, admin edit, deletion, approved correction, standard schedule, automatic clock-out…), <em>author</em> and <em>reason</em>, with the previous value struck through next to the new one.</li>
          <li><strong>Day dossier</strong> (document icon) — the employee's whole day on one screen: every stamp (deleted ones included) with its own history, the justified anomalies with their note, and the correction requests for the same date. It is also reachable from the <strong>Anomalies</strong> page, next to each row.</li>
        </ul>
        <p>From the dossier, <strong>Download PDF</strong> produces an A4 statement with the company header, employee, day, generation date and the name of whoever generated it: this is the document to attach to a contestation. Exporting someone else's dossier is recorded in the <strong>Activity log</strong>.</p>
        <div class="callout callout-info">
          The history is <strong>append-only</strong>: no row can be altered or removed after the fact, not even by an administrator. That property is what makes the statement defensible.
        </div>
      </div>
      ...
      <div class="feature">
        ...
        <p>Deleting is always a <strong>soft delete</strong>: the stamp leaves the totals and the attendance exports, but stays recoverable via <strong>Show deleted</strong>, in the Day dossier and in the <em>Rettifiche</em> sheet of the Excel export.</p>
        <div class="callout callout-warn">
          Every manual action is recorded in the audit log. Always enter a clear reason: it serves both the employee and in case of inspections — it is the text that will appear in the history and in the PDF statement.
        </div>
        <div class="callout callout-info">
          The employee is told: a time change or a deletion made by an administrator raises a notification in the app's bell (no push, no email, so bulk corrections stay quiet) and shows up in their stamps together with the original value.
        </div>
      </div>
    ...
    <section class="chapter" id="web-admin-anomalie">
      ...
      <div class="feature">
        ...
        <table>
          ...
          <tbody>
            ...
            <tr><td><strong>Insert holiday</strong></td><td>Creates the holiday on the day of the anomaly on the employee's behalf. Already approved; the hours are calculated from the assigned shift. It covers the <strong>whole day</strong>, not the single anomaly.</td></tr>
            <tr><td><strong>Insert leave</strong></td><td>Creates an hourly leave. The proposed window covers the period not worked (gap) <strong>of the day</strong>, adjustable with the −/+ buttons in 15-minute steps <strong>inside the blocks</strong> of the assigned schedule. On <strong>split shifts</strong> the unpaid gaps between the blocks are excluded (see below).</td></tr>
        ...
        <p><strong>Holiday and leave apply to the day, not to the single row.</strong> One day can raise several anomalies: <em>Insert holiday</em> and <em>Insert leave</em> appear on <strong>every</strong> row of that day — break rows included. The window proposed for the leave, instead, depends on the row you open the correction from:</p>
        <ul class="tidy">
          <li>on the rows where a <strong>real stamp</strong> delimits the hole — <em>Late clock-in</em> (the entry stamped late) and <em>Early clock-out</em> (the exit stamped early) — the window stays the row's own and does not change: with a clock-in at 10:30 on a 09:00–13:00 / 14:00–18:00 schedule, the <em>Late clock-in</em> row proposes 09:00–10:30, exactly the uncovered hours.</li>
          <li>on every other row — <em>Missing clock-in</em>, <em>Missing clock-out</em>, <em>Insufficient daily hours</em>, break, clock-out out of area, rest day — the window is inherited from the <strong>day</strong> and derived from the whole set of its anomalies: the gap at the start of the shift if there is a late or missing clock-in, the gap at the end of the shift if there is an early or missing clock-out and, when the day only reports <em>Insufficient daily hours</em>, the hours missing at the end of the shift. The summary always states which anomaly the window comes from, so that <em>"from 14:30 to 17:00"</em> on a break row is not read as the length of the break.</li>
          <li><em>Insufficient daily hours</em> belongs to this second group because it says <strong>how many</strong> hours are missing, never at which end of the shift they are missing: on a day with a late clock-in that row proposes the gap at the start of the shift, not the last hours of the shift, which the stamps prove were worked.</li>
          <li><em>Missing clock-in</em> and <em>Missing clock-out</em> belong to the same group because the stamp that would delimit the hole is precisely the one that is missing: on their own they could only point at the whole shift. With a clock-in at 10:30 and no exit stamped, the <em>Missing clock-out</em> row therefore proposes 09:00–10:30 just like the <em>Late clock-in</em> row, not the whole day: the absent stamp is added with <em>Standard stamping</em>, while the leave covers only the hours the stamps prove uncovered.</li>
        </ul>
        <p>Example: clock-in 07:58 and clock-out 14:38 on an 08:00–12:00 / 13:00–17:00 schedule with no lunch break stamped. The day raises several anomalies, among them <em>Lunch break too short</em>: starting from that row too you reach the leave for the day's unworked hours, which are the ones after the early clock-out (14:30–17:00) and not the length of the break.</p>
        <p><strong>Days unworked at both ends.</strong> When the same day has a late clock-in <em>and</em> an early clock-out, there are <strong>worked hours</strong> between the two holes: the unworked periods are two and distinct, one leave cannot cover them, and picking one would mean recording the other as worked. Only in that case is the leave not offered at day level — neither on the rows that inherit the window from the day, <em>Insufficient daily hours</em> and the break rows included, nor in the bulk correction bar — and it has to be entered with <strong>Correct</strong> on the two rows concerned (<em>Late clock-in</em> and <em>Early clock-out</em>), one per period; the app says so and states how many such days there are. <em>Insert holiday</em> stays available instead, because it covers the whole shift and does not depend on where the gap is.</p>
        <p>Two flagged ends are not enough on their own: what counts is whether the <strong>stamps</strong> prove someone was present in between. A day with <strong>no stamps at all</strong> raises both <em>Missing clock-in</em> and <em>Missing clock-out</em>, yet the unworked period is a <strong>single</strong> one — the whole shift — and the leave stays available, in bulk too. In the same way, when only one end is anchored by a real stamp the window is that half: clocked in at 10:30 and never clocked out proposes <em>09:00–10:30</em> (the missing exit is what <em>Standard stamping</em> is for), not the whole day — and it proposes the same window whichever row of the day you open the correction from, <em>Missing clock-out</em> and the bulk correction bar included.</p>
        <p><strong>Split shifts (two or more blocks).</strong> On a schedule such as 08:00–12:00 / 13:00–17:00 the gap between the blocks is <strong>not expected time</strong>: it is neither work nor absence, and it must not come off the leave balance. The proposed window is therefore <strong>trimmed to the blocks</strong> and the leave is inserted <strong>once per block</strong>. Example: clock-in at 09:00 and clock-out at 12:00 on a 09:00–13:00 / 14:00–18:00 schedule proposes <em>12:00–13:00</em> and <em>14:00–18:00</em>, that is <strong>5 hours</strong> — the very hours the <em>Insufficient daily hours</em> anomaly reports as missing — and not the 6 hours from 12:00 to 18:00. The summary lists the blocks and <em>Duration</em> shows their sum; the employee's <em>Requests</em> and the exports therefore show two leave rows for that day. The same holds for the bulk correction bar, which states before you confirm how many selected days have a split shift. The window's two ends are pulled inside the blocks too: a clock-out at 13:30 proposes <em>14:00–18:00</em>, because the half hour after 13:00 falls in the unpaid gap.</p>
        <p>For the same reason the <strong>−/+</strong> buttons move <strong>inside the blocks only</strong>: on 09:00–13:00 / 14:00–18:00, pressing <em>From +</em> takes the window straight from 12:45 to 14:00, jumping the unpaid gap whole, and it cannot stop at 13:30 — a time nobody is scheduled at, which would be discarded at insertion anyway. At the outer edges of the shift the buttons are disabled. <em>From</em>, <em>To</em> and <em>Duration</em> therefore always describe exactly the leave that will be inserted.</p>
        <p>The leaves of a split-shift day are also inserted as <strong>one single operation</strong>: either they all go in or none of them does, and the employee receives <strong>one notification</strong> for the day. If the insertion is refused — because the blocks together would exceed the day's expected hours, for instance when a leave is already on it — the day stays exactly as it was, with no half leave to remove by hand. In that case the anomaly list is reloaded and the proposed window starts again from the day's real state, because the one you had been working on was computed against a state the server has just contradicted.</p>
        <p>One refusal reads differently, though: <em>"another operation on this employee's absences is still running, try again in a moment"</em>. It happens when a bulk correction — or another administrator — is writing absences for the same person. Nothing was written and nothing changed, so the window you tuned by hand <strong>stays exactly as it is</strong> and <em>Confirm</em> retries the very same thing. More generally the window on screen is always the day as it stands now: if the list reloads for any reason — another row corrected, a bulk correction, the <em>Refresh</em> button — and that day's stamps have changed, the open panel starts again from the new proposal; if the day has not changed, your −/+ adjustments stay where you put them.</p>
        <p><strong>When holiday and leave do not appear.</strong> If the day has <strong>no unworked time</strong>, only <strong>justify with a note</strong> stays available. That is the case for days whose only anomaly is about the break (e.g. <em>Break too long</em>, <em>Lunch break outside window</em>) while the hours were completed anyway: booking a whole day of holiday on a day actually worked would not fix the payslip, it would falsify it. The same holds for <em>Work on a rest day</em> and <em>Clock-out out of area</em>, which have no expected schedule to derive the window from.</p>
        <p><strong>Leftover minutes nothing can cover.</strong> A leave must be a multiple of <strong>15 minutes</strong> and is trimmed to the blocks: when the unworked time left is shorter than a quarter of an hour — or does not fall on a quarter of an hour — there is no window to insert at all, and <em>Insert holiday</em> and <em>Insert leave</em> disappear from <strong>every</strong> row of that day. It is a property of the <strong>day</strong>, not of the single anomaly: the <strong>Correct</strong> panel says so and lists the minutes left uncovered on <strong>any</strong> row of that day, break rows and <em>Clock-out out of area</em> included, since they lose the same two actions for the same reason. The bulk correction bar counts how many selected days are in that state. It happens when the schedule's blocks do not start and end on a quarter of an hour: on a 09:00–13:20 / 14:00–17:40 schedule a day of absence covers 465 of the 480 expected minutes and leaves <em>13:15–13:20</em> and <em>17:30–17:40</em> uncovered, which keep raising <em>Missing clock-in</em> and <em>Missing clock-out</em>. The panel also names the way out that row actually has: <em>Standard stamping</em> where a stamp is missing, otherwise — when the stamps are all there, as on a break row or an <em>Early clock-out</em> of a few minutes — the <em>note</em> alone. Either way, fix the block times under <strong>Work schedules</strong> so those minutes do not come back every day.</p>
        <p>On the rows that reach holiday and leave only through the day (break, clock-out out of area, rest day) the menu still opens on <em>Justify with a note</em>: absences stay an explicit choice, never the preselected entry. The same holds for the <strong>bulk correction bar</strong>: when the selection is made only of rows of that kind — five <em>Lunch break too short</em>, say — the bar opens on <em>Justify with a note</em> and not on <em>Insert holiday</em>.</p>
      ...
      <div class="feature">
        ...
        <ul class="tidy">
          <li><em>Justify with a note</em> and <em>Standard stamping</em> are offered <strong>only when valid for every</strong> selected anomaly (the intersection): the note always is, standard stamping only when a stamp is missing on all of them.</li>
          <li><em>Insert holiday</em> and <em>Insert leave</em> are <strong>day-level</strong> corrections and follow a different rule: they are offered when <strong>at least one</strong> selected day can take them, and the days that cannot are <strong>skipped</strong>. Before you confirm, the bar states how many it skips and how many it corrects. A single day with no unworked time — or unworked at both ends, which one leave cannot cover — therefore no longer removes the action from all the others: it used to take one absent Tuesday inside a selected month to lose <em>Insert leave</em> on every other day. The skipped days have to be corrected on their own, from their rows, or justified with a note.</li>
          ...
          <li><em>Insert leave</em> <strong>is available in bulk too</strong>: the time window is not shared but computed <strong>for each day</strong> from the period not worked (gap), so every day gets its own times. To fine-tune it by hand use the <strong>per-row</strong> correction, where the −/+ buttons in 15-minute steps remain. On days with a <strong>split shift</strong> one leave per block is inserted, leaving the unpaid gap out: the bar states how many such days there are before you confirm, and each day is inserted as one single operation (all of its blocks or none), so a refused day stays untouched and shows up in the list of failures.</li>
          <li><strong>Explicit confirmation for bulk holiday.</strong> <em>Insert holiday</em> is the one action in the bar that spends a balance and sends a notification for every day, so a checkbox stating <strong>how many whole days</strong> you are inserting has to be ticked. The <em>Correct</em> button stays disabled until you tick it, and the tick clears if you change action or selection.</li>
    ...
    <section class="chapter" id="web-admin-ferie">
      ...
      <div class="callout callout-warn">
        <strong>Sick leave</strong> requests are auto-approved at the moment of creation and mandatorily require the INPS protocol. A sick leave covering <em>every</em> day of an already-approved holiday makes the "Replaced by sick leave" badge appear on the holiday. If it covers only part of it, the holiday is shortened to the remaining days; if the sick leave falls <em>inside</em> the holiday period, the holiday is split into two requests — the days before and the days after the sick leave — both keeping the original's approver and, as a rule, its status. One exception: if a <strong>cancellation request</strong> was open on the holiday, it stays on the request it was asked about and the days after the sick leave go back to <em>approved</em>, so the employee can ask to revoke them separately. Holiday hours the certificate does not cover therefore stay with the employee.
      </div>
    ...
    <section class="chapter" id="web-admin-esportazioni">
      ...
      <div class="feature">
        ...
        <p>The downloaded file gets a meaningful name:<br />
          <code>presenze-&lt;company&gt;-&lt;from&gt;_&lt;to&gt;-&lt;generated on&gt;.xlsx</code><br />
          for example <code>presenze-rossi-srl-2026-08-01_2026-08-31-20260903-1223.xlsx</code>. The timestamp is when the file was <em>generated</em>, not downloaded: re-downloading the same job always yields the same name, while two extractions of the same period made at different times stay distinguishable. The <strong>Centro Paghe</strong> format keeps the name its specification mandates (<code>ORARIO_&lt;company code&gt;_&lt;MMYYYY&gt;.TXT</code>).</p>
        <div class="callout callout-tip">
          The table refreshes automatically every 2 seconds as long as there are queued or processing jobs. The <strong>Refresh</strong> button above the table forces an immediate reload and shows the time of the last refresh.
        </div>
      </div>
      ...
      <div class="feature">
        ...
        <ul class="tidy">
          ...
          <li><strong>Metadata</strong>: period, generation date, counts and the <strong>dictionary of every column</strong> — sheet by sheet, what each column holds. It is the sheet to read first if the accountant is the one opening the file.</li>
        </ul>

        <div class="callout callout-info">
          <strong>Ore lavorate</strong> (hours worked) and <strong>Ore originali</strong> (original hours) sit side by side on purpose.
          <em>Ore lavorate</em> is the payroll-bearing figure: computed on the <strong>current</strong> stamps (already corrected) and excluding deleted ones.
    ...
    <section class="chapter" id="web-admin-assistenza">
      ...
      <div class="feature">
        ...
        <p>When a request is closed — by you or by support — the row shows <strong>only the outcome</strong> («Resolved by you», «Closed») and not the last handling state: a request closed while it happened to be "waiting for your reply" would otherwise keep asking you for a reply nobody needs.</p>
        <p>The <strong>History</strong> button inside a request opens the list of every state change with its date and time: when it was opened, taken in charge, moved to waiting-on-you, resolved or closed. The system records each change as it happens, so the list is complete even for the steps made from the console.</p>
    ...
    <section class="chapter" id="web-user-stamps">
      ...
      <div class="feature">
        <h3>The table</h3>
        <p>Columns: <strong>When</strong> (date and time), <strong>Event</strong>, <strong>Source</strong>, <strong>Notes</strong> and <strong>Actions</strong>.</p>
      ...
      <div class="feature">
        <h3>If one of your stamps was modified</h3>
        <p>Stamps an administrator moved or deleted <strong>stay visible here</strong>, showing what you actually stamped. If the period contains at least one, a notice with the count appears at the top of the page.</p>
        <ul class="tidy">
          <li><strong>modified</strong> badge: next to the current time, in amber and in brackets, the time you had stamped.</li>
          <li><strong>deleted</strong> badge: the row is struck through and the notes carry the reason given by the administrator.</li>
          <li><strong>Clock</strong> icon — opens the stamp's <strong>history</strong>: who intervened, when and with what reason.</li>
          <li><strong>Document</strong> icon — opens the day's <strong>dossier</strong> (all your stamps of that day with their history) and lets you download it as a PDF.</li>
        </ul>
        <p>You also get a notification in the mobile app's bell on every change or deletion made by an administrator. If you disagree with an amendment, submit a <em>correction request</em> with your reason.</p>
      </div>
    </section>
    ...
    <section class="chapter" id="mob-user-timbra">
      ...
      <div class="feature">
        ...
        <div class="callout callout-info">
          <strong>Only the clock-in is bound to the area.</strong> The position check blocks the <strong>Clock in</strong> only: that is the moment being on site is verified. Every later stamp of the shift (breaks, lunch, clock-out) always goes through, from any position — so you can move between locations, or close from home a shift you left open. If the position is not inside the radius of any assigned location, the stamp is still saved but marked <strong>out of area</strong>: for the clock-out you will see the notice <em>"Clock-out out of area"</em> and an <strong>anomaly</strong> visible to the administrator is raised. If instead you are inside the radius of another location you are assigned to, the stamp is recorded against that location and raises no anomaly. <strong>Smart-working</strong> locations have no area check: they never raise anomalies.
        </div>
      </div>
    ...
    <section class="chapter" id="mob-user-storico">
      ...
      <div class="feature">
        <h3>Stamps modified or deleted by the administrator</h3>
        <p>If an administrator moves the time of one of your stamps or deletes it, the History tells you — this is where you check what you actually stamped.</p>
        <ul class="tidy">
          <li>The day card shows <strong>"Modified or deleted stamps"</strong> in amber.</li>
          <li>Expanding it, a moved stamp carries <em>"You stamped 08:47 · modified by …"</em> underneath.</li>
          <li>Deleted stamps stay in the list, struck through and with a bin icon, along with the reason given by the administrator. They do not count towards the hours.</li>
        </ul>
        <p>You also get a <strong>bell</strong> notification on every change or deletion: tapping it lands you right here. If the amendment does not look right, open the <strong>Corrections</strong> tab and submit a request.</p>
      </div>
    </section>
    ...
    <section class="chapter" id="cantieri">
      ...
      <div class="feature">
        ...
        <p>The <strong>Cantieri dashboard</strong> shows a card per site with the number of activities, workers involved and travel/work totals. At the top you filter by <strong>work site</strong> and pick the period: <strong>by day</strong>, <strong>by month</strong> or <strong>all time</strong>. The <strong>Only with activities</strong> checkbox hides the sites with no entries in the selected period: with many sites it is the quick way to see, on a given day, only those that had movement (the work-site filter narrows down accordingly). In the two bounded modes the ‹ › arrows step one day or one month, while the controls in the middle jump straight to a specific period: a <strong>calendar</strong> in day mode, <strong>month</strong> and <strong>year</strong> dropdowns in month mode; <strong>Today</strong> and <strong>Current month</strong> return to the current period. Opening a card shows the individual entries of the chosen period. From each card, using the dedicated icons, you can <strong>download the PDF</strong> monthly report or <strong>send it by email</strong> (the report is always monthly, so those actions only appear in "by month" mode). When sending you set the <strong>recipients</strong> and, optionally, <strong>CC</strong> and <strong>BCC</strong> addresses and a formatted <strong>note</strong> that appears in the message body; the PDF stays attached.</p>
      </div>
    ...
    <section class="chapter" id="geofence">
      ...
      <div class="feature">
        <h3>Geofence</h3>
        <p>For each branch the admin defines GPS coordinates and — optionally — a <strong>radius</strong> in metres. When the radius is active, the <strong>clock-in</strong> is valid only if you are within this circular area: outside the radius you see the message <em>"You are outside the allowed area"</em> and the stamp is rejected.</p>
        <p>The block applies to the <strong>clock-in only</strong>. Breaks, lunch and the clock-out are always accepted, from any position: if you are not inside the area of any assigned branch the stamp is saved and marked <strong>out of area</strong> — for the clock-out this raises the <em>Clock-out out of area</em> anomaly, with the distance from the branch.</p>
      </div>

      <div class="feature">
        <h3>Changing branch during the shift</h3>
        <p>You can clock in at one branch and carry on (breaks, clock-out) from another branch you are assigned to. During the shift the branch of each stamp is <strong>detected from your position</strong>: if you are inside the radius of another assigned branch, the stamp is recorded against that branch and raises no anomaly, whatever branch is selected in the app. If you are near no branch at all, the stamp stays attributed to the branch where you opened the shift and is marked out of area.</p>
      </div>
      ...
      <div class="feature">
        ...
        <ul class="tidy">
          ...
          <li><strong>Block</strong> — the <strong>clock-in</strong> is rejected. Later stamps of the shift (breaks, lunch, clock-out) still go through, flagged: a suspicious position must not stop you from closing a shift already open.</li>
        </ul>
      </div>
    </section>
    ...
    <section class="chapter" id="faq">
      ...
      <div class="feature">
        ...
        <p>From the mobile app → <strong>Requests</strong> tab → <strong>+</strong> → select <strong>Sick leave</strong>. Enter the INPS protocol and the dates. The request is auto-approved and covers any overlapping holiday: the holiday days the certificate does not cover stay yours and keep showing among your requests, split into two periods (before and after the sick leave) if needed.</p>
      </div>
