...
| Timbrature         | `/timbrature`      | hero text `Ore lavorate` / `Ore conteggiate` / `Entrata` / `Pause` / `Uscita`, action buttons `Timbra ingresso` / `Timbra uscita` / `Inizia pausa` / `Termina pausa`, `getByLabel('Profilo')` for header avatar. `Ore conteggiate` floors to 15-min blocks (counted-day.ts). |
...
| Richieste          | `/richieste`       | tab pills `Le mie` / `Da approvare` (use `getByText`). Quota card: text `Ferie` / `Permessi` + `{X}h` + `(X.XXh dopo richieste in attesa)`. **FAB (employee only):** `getByLabel('Nuova richiesta').first()`. New-request modal: `Tipo` row with `Ferie` / `Permesso` / `Malattia` / `Assenza`, sections `Dal` / `Al` / `Durata` (only ferie+permessi), pills `Tutto il giorno` / `Orario specifico`, `Ora inizio` / `Ora fine`, malattia-only `Numero protocollo INPS` with placeholder `es. 1234567890`, `Note (facoltative)` (or `Motivazione (facoltativa)` for assenza — never `(obbligatoria)`), assenza-only `Tipologia di assenza` picker + `Retribuita` / `Non retribuita`, submit text `Invia richiesta` (ferie/permessi/assenza) or `Invia segnalazione` (malattia). Hint: `Disponibili: {X}h · {Y}h dopo richieste in attesa`. |
