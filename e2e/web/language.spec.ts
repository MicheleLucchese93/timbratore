...
// This test switches to English and ALWAYS restores Italian in `finally`, so the
...
test.describe('web — language switcher', () => {
  test('Settings select switches the UI to English and back to Italian', async ({ page }) => {
    await page.goto('/settings');
    await expect(page.getByRole('heading', { name: /Impostazioni/i })).toBeVisible({ timeout: 15_000 });

    // The interface-language <select> lives only in Settings now (the sidebar
    // toggle was removed). Locate it by its option *values* (it/en) so the
    // locator survives the UI relabelling its options when the language flips.
    const langSelect = page.locator('select:has(option[value="it"]):has(option[value="en"])');
    await expect(langSelect).toBeVisible({ timeout: 10_000 });

    // selectOption fires a fire-and-forget PATCH /me to persist the choice. Two
    // rapid switches (EN then IT) race: the test only asserts the instant local
    ...
    try {
      ...
      await patchEn; // 'en' persisted before we switch back
