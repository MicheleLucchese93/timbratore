...
- CORS and data-access logging are disabled. Application access records therefore remain distinct from a full storage-operation audit trail.
...
| Cache persistence | `private, no-store` on document APIs, new R2 writes, and signed GET response override; metadata-only backfill script for existing documents. | Local API checks pass. A disposable R2 object confirmed signed-response override and conditional self-copy support with identical ETag/size; it was deleted. Existing-document backfill pending deployment. |
...
- Application migration 066, new API image and existing-object cache backfill: pending rollout verification.

## Remaining decisions and limits

1. Finish dedicated bucket-scoped application credential provisioning and review the owners/rotation of shared account administrator credentials.
...
3. Decide whether EU jurisdiction and R2 data-access logging are required. Current location is EEUR, default jurisdiction; encryption does not establish residency or complete audit coverage.
