...
### Dedicated credential cutover

The user created the prepared token and authorized installation on 10 September 2026. Cloudflare's verification endpoint confirmed it was active with no automatic expiry; the ID matched the Chrome creation result for `sonoqui-prod-documents`. Its S3 credentials were derived using Cloudflare's documented token-ID/SHA-256 mapping. No token, secret or signed URL is included in this report. [R2 authentication](https://developers.cloudflare.com/r2/api/tokens/).

The candidate passed positive storage tests and negative permission tests before installation. Only `R2_ACCESS_KEY_ID` and `R2_SECRET_ACCESS_KEY` changed in the production environment, retaining mode `0600`. The API was recreated with the same application image and passed health checks in five seconds. The Penno container/environment and infrastructure environment were verified unchanged.
